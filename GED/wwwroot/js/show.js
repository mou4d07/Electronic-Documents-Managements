document.addEventListener('DOMContentLoaded', function () {
    ...
    if (viewerElement) {
        var viewer = OpenSeadragon({
            ...
            tileSources: {
                type: 'image',
                url: viewerElement.getAttribute('data-jpeg-path')
            },
            showNavigator: true,
        ...
        viewer.addHandler('open', function () {
            ...
            var downloadButtonElement = document.createElement('div');
            downloadButtonElement.id = 'download-viewer-button';
            downloadButtonElement.className = 'download-button-osd';
            //downloadButtonElement.innerHTML = '<i class="fas fa-download"></i>';
            downloadButtonElement.title = 'Download Plan';

            var downloadButton = new OpenSeadragon.Button({
                tooltip: 'Download',
                onRelease: function () {
                    var downloadPath = viewerElement.getAttribute('data-jpeg-path');
                    // Force download by adding the parameter
                    var url = new URL(window.location.origin + downloadPath);
                    url.searchParams.set('download', 'true');
                    window.location.href = url.toString();
                },
                element: downloadButtonElement
            });
            viewer.addControl(downloadButton.element, { anchor: OpenSeadragon.ControlAnchor.BOTTOM_RIGHT });
